...

{
  ...
  # One Set<KClass<out Mutant>> per mutation operator actually observed (see
  ...
  for op in $(printf '%s\n' "${!operator_indices[@]}" | sort); do
    ...
    echo "  /** All mutants generated by mutant-kraken's [$op] operator. */"
    echo "  val $prop_name ="
    echo "      setOf("
