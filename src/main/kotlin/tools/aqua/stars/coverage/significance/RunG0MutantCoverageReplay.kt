...
/**
 * @param args Optional `--runId=<id>` to restrict the analysis to one evaluation run. Without it,
 *   every run's ticks flagged with `next_tick_monitor_g0_Accidents_failed = true` are replayed.
 */
fun main(args: Array<String>) {
  val runId = CliArgs.optionalInt(args, "runId")

  DbBootstrap.connectAndCreateSchema(DbBootstrap.DbConfig(port = 5432))
  G0MutantCoverageReplayAnalysis.evaluate(runId)
  println("Finished!")
}
