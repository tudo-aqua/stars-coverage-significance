...
fun writeCSVAndTeXFiles(
  ...
  writeTeXFile(metricName = metricName, "countOfKilledMutants", selectedMonitors, numberOfMutants)
  writeCSVFiles(
      metricName = metricName,
      "countOfFailedMonitors",
      selectedMonitors,
      map.map { it.key to it.value.countOfFailedMonitors.getBoxPlotValues() }.toMap())
  writeTeXFile(metricName = metricName, "countOfFailedMonitors", selectedMonitors, numberOfMutants)
  writeCSVFiles(
      metricName = metricName,
  ...
  writeTeXFile(
      metricName = metricName, "countOfDistinctMonitorsFailed", selectedMonitors, numberOfMutants)
}
    ...
    \begin{tikzpicture}
        ...
        \begin{axis}[
            ...
            \pgfplotsinvokeforeach{0,...,\TotalRows}{
                ...
                coordinates {};
                \addplot[only marks, mark size=.2ex, mark=o, color=blue] table {\datatableMildOutliers};
                \addplot[only marks, mark size=.2ex, mark=asterisk, color=red] table {\datatableExtremeOutliers};
            }
        \end{axis}
    \end{tikzpicture}
\end{document}     
