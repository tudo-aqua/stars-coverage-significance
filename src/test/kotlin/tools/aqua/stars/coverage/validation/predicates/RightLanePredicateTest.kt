...
import tools.aqua.stars.coverage.significance.tsc.isBehindOnRightLane
import tools.aqua.stars.coverage.significance.tsc.isInFrontOnRightLane
...
/** Test class for right lane predicates. */
class RightLanePredicateTest {
  ...
  fun `Test vehicle in front on right lane at max distance`() {
    ...
    val tick = getTestTimeStep(listOf(frontVehicle, behindVehicle))
    assertFalse(isInFrontOnRightLane.holds(tick, frontVehicle to behindVehicle))
  }
  ...
  fun `Test vehicle behind on right lane at min distance`() {
    ...
    val tick = getTestTimeStep(listOf(frontVehicle, behindVehicle))
    assertFalse(isBehindOnRightLane.holds(tick, behindVehicle to frontVehicle))
  }
