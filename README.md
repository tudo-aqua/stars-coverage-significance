...
- Were original mutants "killed again" on replay? (`originalMutantReproducedCount` / `originalMutantNotReproducedCount` / `originalMutantInconclusiveCount`, each paired with a `*TickIds` list of the matching tick ids, plus a per-mutant breakdown in `mutantStats`)
