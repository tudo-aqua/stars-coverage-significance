#!/usr/bin/env python3
"""Combined scatter + box plots for tick-sampling strategies across all suite sizes.
...
subdirectories, producing:
  - Per size (full strategies):  run_<id>/size_<n>/draw_ticks_combined.[png|pdf]
  - Per size (rare strategies):  run_<id>/size_<n>/draw_ticks_combined_rare.[png|pdf]
  - All sizes (full strategies): run_<id>/draw_ticks_combined_all.[png|pdf]
  - All sizes (rare strategies): run_<id>/draw_ticks_combined_all_rare.[png|pdf]
...
starting scenarios (contrast with baseline_next_tick_combined.py's scenario-based strategies).
"""
...
def _process_run(base: Path, run_id: int) -> None:
    ...
    for size, size_dir in size_dirs:
        for group_suffix, group_strategies in STRATEGY_GROUPS:
            ...
            for ext in (".png", ".pdf"):
                fig.savefig(
                    size_dir / f"draw_ticks_combined{group_suffix}{ext}",
                    bbox_inches="tight",
                )
            plt.close(fig)
    ...
    for group_suffix, group_strategies in STRATEGY_GROUPS:
        ...
        fig.tight_layout()
        out = base / f"draw_ticks_combined_all{group_suffix}"
        for ext in (".png", ".pdf"):
